...
def process_image(input_path: str):
    ...
    if lines is None or len(lines) < 2:
        raise ValueError("Not enough lines were detected.")
    ...
    if not vertical_lines or not diagonal_lines:
        raise ValueError("Could not find both a trunk and a branch.")
    ...
    if abs(sweep_forward - final_angle_value) < 1:
...
if __name__ == "__main__":
    ...
    try:
        angle, result_img = process_image(input_image_path)
        cv2.imwrite(output_image_path, result_img)
        print(f"Processing complete. Angle: {angle:.1f}. Result saved to: {output_image_path}")
